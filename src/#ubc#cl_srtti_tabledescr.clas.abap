...
CLASS /ubc/cl_srtti_tabledescr DEFINITION
  PUBLIC
  INHERITING FROM /ubc/cl_srtti_complexdescr
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA line_type TYPE REF TO /ubc/cl_srtti_datadescr .
...
CLASS /ubc/cl_srtti_tabledescr IMPLEMENTATION.
  ...
  METHOD get_rtti.
    ...
    TRY.
      ...
      CATCH cx_sy_table_creation INTO DATA(error).
        RAISE EXCEPTION TYPE /ubc/cx_srtti EXPORTING previous = error.
    ENDTRY.
  ...
  METHOD constructor.
    ...
    line_type       = CAST #( /ubc/cl_srtti_typedescr=>create_by_rtti( rtti->get_table_line_type( ) ) ).
